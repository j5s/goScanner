...
func worker(ips chan string, wg *sync.WaitGroup) {
	for ip := range ips {
		...
		if err != nil {
			...
			println(fmt.Sprintf("%s -> %s", ip, rst))
			result = append(result, NbtScanResult{Target: ip, Message: rst})
		}
		wg.Done()
	}
}

type NbtScanResult struct {
	Target  string
	Message string
}

var result []NbtScanResult

// 主要外部调用函数
func NbtScan(ips []string, threadCount int) []NbtScanResult {
	result = []NbtScanResult{}
	addrs := make(chan string, threadCount)
...
func NbtScanTest() {
	...
	for index := range ipList {
		ip := fmt.Sprintf("10.5.84.%d", index)
		go func() {
			result, err := Scan(ip)
			if err != nil {
			} else {
				println(fmt.Sprintf("%s -> %s", ip, result))
			}
			wg.Done()
		}()
	}
